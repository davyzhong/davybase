...
class QwenProvider(LLMProvider):
            ...
            try:
                ...
                async with httpx.AsyncClient(timeout=httpx.Timeout(30.0, read=600.0)) as client:
                    ...
                    if response.status_code == 429:
                        ...
                        logger.warning(f"千问 API 触发限流，等待 {retry_after} 秒（第 {attempt+1}/{max_retries} 次重试）")
                ...
                if attempt < max_retries - 1:
                    ...
                    logger.warning(f"千问 API 读取超时，等待 {retry_after} 秒后重试（第 {attempt+1}/{max_retries} 次）")
                    await asyncio.sleep(retry_after)
                else:
                    raise RuntimeError(f"千问 API 调用失败：超过最大重试次数（最后错误：读取超时）")
        raise RuntimeError("千问 API 调用失败：超过最大重试次数")
    ...
    async def digest_note(self, content: str) -> DigestResult:
        """消化单条笔记：生成标题、分类、标签"""
        prompt = DIGEST_PROMPT.format(content=content[:4000])
        messages = [{"role": "user", "content": prompt}]
        ...
        try:
            ...
            return DigestResult(
                title=result.get("title", "未命名笔记")[:50],
